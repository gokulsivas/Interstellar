...
const TimeSimulation = () => {
  ...
  const handleSimulate = async () => {
    if (!numOfDays && !toTimestamp) {
      setError("Please provide either number of days or target date");
      return;
    }
    ...
    try {
      const response = await simulateDays({
        ...
        itemsToBeUsedPerDay: itemsToBeUsed.length > 0 ? itemsToBeUsed : undefined,
      });
